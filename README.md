...
`--no-overlay` и `--in-place` **взаимоисключаемые** с overlay-режимом:
- `--no-overlay` → кэш + CSV, без в-place apply, без оверлея.
- `--in-place` → apply прямо в .mod (EN-оригинал модифицируется).
- Без этих флагов → **overlay-режим** (Workshop) / в-place (встроенные).
