...
> ⚠️ **ВНИМАНИЕ — v2.1+ (включая 2.x): логика переводов сильно изменилась.**
> В этой ветке отбор «переводить/не переводить» строки стал **по object-ID из `.po`**,
...
>
> **Перед первым запуском перевода в v2.x выполни:**
> ```
...
| **Полная очистка v2.x** (все моды + CSV новый/legacy + весь кэш + оверлеи + EN-бэкапы) | `./revert_mods.bat --full-clean` |
| ...то же, но ОСТАВИТЬ EN-бэкапы `.orig_*.backup` | `./revert_mods.bat --full-clean --keep-backups` |
