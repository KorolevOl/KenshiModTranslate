...
1. **Cross-reference** — строка, которая одновременно является `record.Name`
   И значением какого-нибудь поля в том же моде, — идентификатор. Таковые
   исключаем из списка перевода (маркируем как identifier).
...
(описания, диалоги, названия предметов/персонажей/городов) переводится.
