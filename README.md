...
- **`--lines` / `--text`** — точечный перезапуск только строк:
  `./translate_mods.bat "мод" --lines "12,40-55,99"` или `--text "Whetbone"`.
  Всё прочее мод оставляет нетронутым, не ходит к LLM заново и не обнуляет кэш.
- **Словарь `dict.json` без дублей** — единый источник правил:
  `exact` = 153 строк/термина («строка целиком = термину»), `words` = 113 имён
  («безопасно вставлять внутри фразы»). Словарь вставляется в промпт
  **один раз** (экономия ~850 токенов на каждый запрос).
- **Адаптация нейронкой** — промпт переписан: онотопея, составные имена,
  слоганы, «английский слоган на русской букве» — LLM больше не копирует
  английские звукоподражания. Словарь = **направление**, не догма.
- **Детерминированная подстановка** — `apply_dict` подставляет `words`
  в порядке **длины ключа** (длинный раньше короткого),
  что убивает порядок случайного обхода `set` (в прошлых версиях
  `PYTHONHASHSEED` влиял на результат).
- **README обновлён** — добавлены разделы «Словарь терминов» с
  примером использования для новичка, «Точечный перевод» с примерами,
  исправлена таблица (были две строки в одной).
...
| Пере-перевести ТОЛЬКО некоторые строки (не весь мод) | `./translate_mods.bat "имя мода" --lines 5` (порядковый номер в CSV) — или `./translate_mods.bat "имя мода" --text "фраза"` (по содержимому, без номера строки) |
...
- **Словарь `dict.json`** — канонические термины: `exact`
  (строка целиком = термину → канон, LLM не спрашивают) и `words`
  (термин внутри фразы → страховка от EN-остатков). В промпт LLM вставляется
  один раз, без дублей (см. раздел «Словарь терминов»).
