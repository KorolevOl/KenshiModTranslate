...
которая подключается сразу после оригинала в `__mods.list` (игра подгружает
обе, оверлей перебивает по object-ID). Обновление мода из Workshop
**ничего не затирает** — оверлей живёт отдельно.
...
Откроется **интерактивное меню**. Шаг 1 — выбрать **область**:

```
1) Steam Workshop            (N модов)
2) Локальные папки kenshi\mods (M модов)
3) Всё вместе                 (N+M модов)
```

Шаг 2 — программа считается и показывает **список модов с количеством
непереведённых строк** (уже переведённые и моды без строк — внизу).
Счёт — по всем источникам RU: твой кэш, RU-близнец-мод
(`… RUS`/`… RU` из Workshop), `.po` игры и словарь. Затем выбираешь,
**какие** моды переводить:

```
1, 5, 9-14        — по номерам из списка
name:crossbow     — по (частям) имени
(all / Enter)     — все непереведённые из области
q                 — выход без перевода
```

Переведённые моды (в списке ниже) **не переводятся повторно**.
Два прогресс-бара (ALL/NOW), счёт — в токенах.
`Ctrl+C` не теряет прогресс — следующий запуск продолжит с того же места.

> Чтобы перевести **всё** без пошагового выбора — `--all`/`--steam`/`--mods`
> (область задаётся флагом, выбор модов по номерам не появляется).
...
└──────────────────┘               └────────────────────────────────────┘
                               ...
                               [вторая строка: оверлей RUS]  ← позже = победа
```
...
- **RU-оверлей** — компактный .mod, содержит только записи,
  ...
  перебивает оригинал.
- **`__mods.list`** — список загрузочных модов. Строка оверлея
  вставляется **прямо после** строки оригинала. Бэкап — рядом
  (`kenshi\data\__mods.list.<ts>.bak`).
- **Один откат** = `python overlay.py uninstall <имя>` — удаляет оверлей
  и строку из списка (бэкап тоже сохраняется).
...
|---|---|
| Перевести ВСЁ (все моды) | `./translate_mods.bat` (меню: 1), или `./translate_mods.bat --all` |
...
| `translate.interactive_select` | **интерактивный выбор** модов (по умолчанию `true`): `translate_mods.bat` без аргументов → список непереведённых + выбор номеров. `false` — старое поведение (меню только области + все моды) |
...
| RU-оверлей не появился в игре | `python overlay.py list` → строка есть в `kenshi\data\__mods.list`? → `kenshi\mods\<имя> RUS\` существует? |
| Оверлей не подключается | Убедись, что строка оверлея **сразу после** оригинала в `__mods.list`. `python overlay.py install "имя"` |
