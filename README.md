...
| Посмотреть/поправить перевод | файл `translate.csv` в папке мода (Excel) → сохранить → `assemble_mod.bat "имя мода"` |
...
Everything above is enough for startup. Here: how it works under the hood, a manual
installation, and finer settings. A new user can skip.
