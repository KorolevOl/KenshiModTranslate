...
def classify_row(en, ru):
    ...
    if cyrillic_words(ru) >= 2 and _mixed_script_words(ru, en) >= 1:
        return "mixed"
...
def _mixed_script_words(s, en=None):
    ...
    ВАЖНО (2026-09-25): LLM может вернуть официальное RU-написание (из game-po),
    где сокращающиеся слова смешивают латынь и кириллицу в рамках слова
    (Kenshi RU: «Oldworld Bow MkI» → «Старый балет MкI» — M латиница, к кириллица).
    Проверяем: если toкен (транслитерация: кириллица→латынь) совпадает по core
    (первые 3 буквы) с каким-либо словом из EN → это НЕ mixed-артефакт,
    а «сознательное сокращение» → пропускаем.
    """
    if not isinstance(s, str):
        return 0
    ...
    # нормализуем EN-слова: lower + транслитерация (кириллица→латынь) для сравнения
    _TRANSLIT = str.maketrans({
        "а":"a","б":"b","в":"v","г":"g","д":"d","е":"e","ж":"zh","з":"z","и":"i",
        "й":"j","к":"k","л":"l","м":"m","н":"n","о":"o","п":"p","р":"r","с":"s",
        "т":"t","у":"u","ф":"f","х":"h","ц":"ts","ч":"ch","ш":"sh","щ":"shch",
        "ъ":"","ы":"y","ь":"","э":"e","ю":"yu","я":"ya","ё":"yo","ё":"e",
        "А":"a","Б":"b","В":"v","Г":"g","Д":"d","Е":"e","Ж":"zh","З":"z","И":"i",
        "Й":"j","К":"k","Л":"l","М":"m","Н":"n","О":"o","П":"p","Р":"r","С":"s",
        "Т":"t","У":"u","Ф":"f","Х":"h","Ц":"ts","Ч":"ch","Ш":"sh","Щ":"shch",
    ...
    if en:
        for w in en.split():
            cn = _core_norm(w)
            if len(cn) >= 2:  # минимум 2 буквы в core, чтобы не гадать
                _en_cores.add(cn)
    n = 0
    for t in (s or "").split():
        ...
        if has_cyr and (has_lat or has_cjk):
            # 2026-09-25: пропускаем токен, если после транслитерации совпадает
            # по core с каким-либо словом из EN (сокращение «MкI»/«MkI»/«МкI»).
            cn = _core_norm(t)
            skip = cn in _en_cores
            if not skip and len(cn) >= 4:
                for w in _en_cores:
                    if w[:4] == cn[:4]:
                        skip = True
                        break
