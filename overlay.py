...
Механика (проверена в игре 2026-09-24 на Medieval_Crossbows):
  ...
  * Строка оверлея вставляется в data\__mods.list СРАЗУ ПОСЛЕ строки оригинала
    (каталог Workshop) И в data\mods.cfg (РЕАЛЬНАЯ включённость; строка
    '<Имя>.mod' сразу после '<Оригинал>.mod'). Оба файла — с автобэкапом
    рядом (kenshi\data\). Раньше вписывали только __mods.list — оверлей
    виделся в лаунчере, но оставался ВЫКЛЮЧЕННЫМ (найден 2026-09-24: код BEEP
    saveLoadOrder/readModsCfg + Steam-тред + корреляция 2/2 на живых оверлеях).
